<template>
    <li class="relative">
        <div :class="{ 'border-red-600 bg-red-300 bg-opacity-20' : $route.name === menu.route || $route.matched[1]?.name === menu.route }"
        ...
        <ul v-if="menu.children !== undefined" class="">
            <li v-for="child in menu.children" :key="child.name">
                <router-link :to="{ name: child.route}" 
                    :class="{ 'bg-red-300 bg-opacity-10' : $route.name === child.route || $route.matched[2]?.name === child.route }"
                    class="lg:px-4 px-3 py-3 flex lg:w-full w-14 text-xs tracking-widest items-center hover:bg-red-200 hover:bg-opacity-20 lg:space-x-3">
